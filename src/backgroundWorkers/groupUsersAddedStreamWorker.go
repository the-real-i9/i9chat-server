	...
	go func() {
		for {
			...
			for _, stmsg := range streams[0].Messages {
				...
				msg.AdminUser = stmsg.Values["adminUser"].(string)
				...
				msg.AdminUserCHE = helpers.FromJson[appTypes.BinableMap](stmsg.Values["adminUserCHE"].(string))
			...
			for i, msg := range msgs {
				groupNewMembers[msg.GroupId] = msg.NewMembers

				gactData := msg.AdminUserCHE

				CHEId := gactData["che_id"].(string)

				newGroupActivityEntries = append(newGroupActivityEntries, CHEId, helpers.ToJson(gactData))

				chatGroupActivities[msg.AdminUser+" "+msg.GroupId] = append(chatGroupActivities[msg.AdminUser+" "+msg.GroupId], [2]string{CHEId, stmsgIds[i]})
